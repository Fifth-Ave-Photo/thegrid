...
Grid Index RSS Importer pulls headlines from external RSS feeds into WordPress as posts. Designed to pair with The Grid Index theme but works as a standalone importer with any theme.
...
1. Upload the plugin folder to `/wp-content/plugins/grid-index-rss-importer/` or install via the WordPress plugin uploader.
2. Activate the plugin via the Plugins menu.
3. Find the plugin under **Grid Index → Grid RSS** (when The Grid Index theme is active) or under its own top-level **Grid RSS** menu.
...
1. Feeds tab — list of active feeds with status, source name, interval, and last-fetched time.
2. Catalog tab — 47 curated feeds organized by section with cards and list views.
3. Settings tab — import frequency, post status, image rules, data persistence preference.
4. Diagnostics tab — feed health check, duplicate detector, import log.
